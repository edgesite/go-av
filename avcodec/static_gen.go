...
func Open(p0 *Context, p1 *Codec, p2 **avutil.Dictionary) int32 {
	...
	a := (*C.struct_AVCodecContext)(unsafe.Pointer(p0))
	a.time_base = *(*C.struct_AVRational)(unsafe.Pointer(&p0.TimeBase))
	a.height = C.int(p0.Height)
	a.width = C.int(p0.Width)
	a.sample_aspect_ratio = *(*C.struct_AVRational)(unsafe.Pointer(&p0.SampleAspectRatio))
	a.pix_fmt = int32(p0.PixFmt)
	a.sample_aspect_ratio = *(*C.struct_AVRational)(unsafe.Pointer(&p0.SampleRate))
	a.sample_fmt = int32(p0.SampleFmt)
	fmt.Println(a.time_base)
