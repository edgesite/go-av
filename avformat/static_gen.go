...
func Dump(p0 *Context, streamIndex int32, url string, isOutput int32) {
	defer runtime.KeepAlive(p0)
	var s1 *C.char
	if url != "" {
		s1 = C.CString(url)
	...
	defer runtime.KeepAlive(streamIndex)
	defer runtime.KeepAlive(isOutput)

	C.av_dump_format((*C.struct_AVFormatContext)(unsafe.Pointer(p0)), *(*C.int)(unsafe.Pointer(&streamIndex)), s1, *(*C.int)(unsafe.Pointer(&isOutput)))
}
